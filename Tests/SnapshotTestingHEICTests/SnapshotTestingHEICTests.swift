...
final class SnapshotTestingHEICTests: XCTestCase {
    ...
    func test_SwiftUI_HEIC_lossless() {
        let view: some SwiftUI.View = SwiftUIView()
        assertSnapshot(of: view, as: .imageHEIC(layout: .device(config: .iPadPro12_9), compressionQuality: .lossless))
    }
    ...
    func test_SwiftUI_HEIC_maximum() {
        let view: some SwiftUI.View = SwiftUIView()
        assertSnapshot(of: view, as: .imageHEIC(layout: .device(config: .iPadPro12_9), compressionQuality: .maximum))
    }
#endif
    ...
    private func createMacOSProfileView() -> NSView {
        ...
        let avatarContainer = NSView(frame: NSRect(
            x: (500 - avatarSize) / 2,
            y: 340,
            width: avatarSize,
            height: avatarSize
        ...
        let avatarInner = NSView(frame: NSRect(
        ...
        let onlineIndicator = NSView(frame: NSRect(
        ...
        let verifiedBadge = NSView(frame: NSRect(
        ...
        for (index, stat) in stats.enumerated() {
            ...
            if index < stats.count - 1 {
                let divider = NSView(frame: NSRect(x: CGFloat(index + 1) * statWidth - 0.5, y: 10, width: 1, height: 40))
